...
async function gotoClean(path, { timeout = 30000 } = {}) {
  ...
  page.on("console", hC);
  await page.goto(BASE + LANG_PREFIX + path, { waitUntil: "networkidle", timeout });
...
try {
  ...
  await page.goto(BASE + LANG_PREFIX + "/", { waitUntil: "load" });
  const h1 = await page.locator("h1").first().innerText();
  R(/last-mile|17\.000|sistem/i.test(h1), "landing: headline", h1.slice(0, 40));
  // check case numbers on landing
  const body = await page.locator("body").innerText();
  // Angka kanonik landing (diselaraskan ke dokumen kasus pada audit konten;
  // daftar lama memuat angka halaman lain seperti 41,5%/≥55 dari KPI Tracker).
  for (const s of ["61,8%", "75%", "17,25", "24,3", "5,5", "57,7%", "20%"]) {
    R(body.includes(s), "landing num: " + s, body.includes(s) ? "" : "MISSING");
  }
  await page.screenshot({ path: `${shotDir}/e2e-landing.png` });

  // ===== 2. Login =====
  await page.goto(BASE + LANG_PREFIX + "/login", { waitUntil: "load" });
  ...
  await page.goto(BASE + LANG_PREFIX + "/dashboard/methodology", { waitUntil: "load" });
  await page.waitForTimeout(2500);
  R(/Audit & Rekonsiliasi Angka|Jumlah hub = 23/.test(await page.locator("body").innerText()), "methodology: audit section");

  await page.goto(BASE + LANG_PREFIX + "/analisis", { waitUntil: "load" });
  await page.waitForTimeout(2500);
  ...
  await page.goto(BASE + LANG_PREFIX + "/dashboard/kurir/overview", { waitUntil: "load" });
  await page.waitForTimeout(1500);
  const fab = page.locator('[data-testid="nigi-fab"]').first();
  ...
  const navbar = await page.locator('nav[data-testid="bottom-nav"] [data-overflow] button, nav[data-testid="bottom-nav"]').count();
