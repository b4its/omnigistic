...
await p.goto("http://127.0.0.1:3077/", { waitUntil: "load" }); await p.waitForTimeout(2500);
console.log("landing loaded-class:", await p.evaluate(() => document.querySelector(".landing")?.className));
console.log("hero line transform:", await p.evaluate(() => (() => { const el = document.querySelector(".line-inner"); return el ? getComputedStyle(el).transform : "(tidak ada .line-inner)"; })()));
...
console.log("marquee anim:", await p.evaluate(() => (() => { const el = document.querySelector(".marquee-track"); return el ? getComputedStyle(el).animationName : "(tidak ada .marquee-track)"; })()));
await p.goto("http://127.0.0.1:3077/dashboard/pusat/executive", { waitUntil: "load" }); await p.waitForTimeout(2200);
