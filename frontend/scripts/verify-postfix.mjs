...
const t0 = Date.now();
await page.goto("http://127.0.0.1:3077/id/dashboard/kurir/cod-risk", { waitUntil: "load" });
...
await page.goto("http://127.0.0.1:3077/id/dashboard/hub/forecast", { waitUntil: "load" });
await page.waitForTimeout(1600);
const c = await page.evaluate(() => { const cv = document.querySelector("canvas"); if (!cv) return [["(canvas belum ada)", 0]]; const ctx2 = cv.getContext("2d"); const img = ctx2.getImageData(0,0,cv.width,200000).data; const seen={}; for (let i=0;i<img.length;i+=4){ if(img[i+3]>200){ const k=img[i]+','+img[i+1]+','+img[i+2]; seen[k]=(seen[k]||0)+1; } } return Object.entries(seen).sort((a,b)=>b[1]-a[1]).slice(0,4); });
console.log("dark chart colors:", JSON.stringify(c));
// chip click → jawaban (kirim) di Nigi AI via role DATA
await page.goto("http://127.0.0.1:3077/id/dashboard/data/overview", { waitUntil: "load" }); await page.waitForTimeout(800);
await page.click('[data-testid="nigi-fab"]'); await page.waitForTimeout(500);
const inSel='[data-testid="nigi-input"]';
await page.fill(inSel, "alamat bisa ambigu");
await page.keyboard.press("Enter");
await page.waitForTimeout(1300);
const chatTxt = await page.locator('[data-testid="nigi-close"]').locator("..").locator("..").innerText();
console.log("DATA chip QA-hit:", /Cibinong|Depok|Rempoa|3 lokasi/.test(chatTxt));
// Escape close & focus return
await page.keyboard.press("Escape"); await page.waitForTimeout(600);
const panelGone = await page.locator('[role="region"][aria-label="Nigi AI"]').count();
console.log("esc closes panel:", panelGone === 0);
// compact bottom nav + safe overlay
await page.setViewportSize({ width: 390, height: 844 });
await page.goto("http://127.0.0.1:3077/id/dashboard/hub/dashboard", { waitUntil: "load" }); await page.waitForTimeout(1500);
const bar = await page.locator('nav[data-testid="bottom-nav"]').isVisible();
