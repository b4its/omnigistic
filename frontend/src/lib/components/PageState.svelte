...
{#if error}
  <div class="rounded-2xl border border-destructive/40 bg-destructive/5 p-6 text-center">
    <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-destructive/10 text-destructive-foreground"><Icon name="warn" cls="h-6 w-6" /></span>
    <p class="mt-3 text-sm font-semibold text-foreground">{errorTitle}</p>
    <p class="mt-1 text-xs text-muted-foreground">{errorHint}</p>
    {#if onretry}
      <button type="button" onclick={onretry} class="mt-3 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground">Coba lagi</button>
    {/if}
  </div>
{:else if loading}
  <div class="space-y-4">
