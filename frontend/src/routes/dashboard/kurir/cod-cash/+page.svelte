<script lang="ts">
  ...
  async function run(userTriggered = false) {
    busy = true;
    if (userTriggered) notify({ message: m.kc2t1(), type: "info", title: "COD Cash" });
    failed = false;
    try {
      ...
      res = null;
      failed = true;
      if (userTriggered) notify({ message: m.kc2t2(), type: "error", title: "COD Cash" });
    }
    loaded = true;
    busy = false;
  }
  ...
  const riskChart = $derived(
    res
      ? barChart(
          ["Manual", m.kc2t3()],
          [
            { name: m.kc2t4(), data: [Math.round(res.risk.discrepanciesPerDayBefore), Math.round(res.risk.discrepanciesPerDayAfter)] },
            { name: m.kc2t5(), data: [Math.round(res.risk.discrepancyCostIdrBefore / 1e6), Math.round(res.risk.discrepancyCostIdrAfter / 1e6)] }
          ],
          { maxBarWidth: 60 }
        )
      : null
...
<div class="space-y-6">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div>
      <h1 class="font-heading text-xl font-semibold tracking-tight">{m.cc01()}</h1>
  ...
  {#if loaded && failed}
    <PageState loading={false} error={true} errorTitle={m.kc2t6()} errorHint={m.kc2t7()} onretry={() => run(true)} />
  {:else if loaded && res}
    <div class="rounded-2xl border border-[color-mix(in_oklab,var(--bitcoin)_30%,transparent)] bg-[color-mix(in_oklab,var(--bitcoin)_6%,transparent)] p-5">
      <p class="text-sm font-semibold">{m.cc03()}</p>
      <p class="mt-1 text-xs text-muted-foreground">{m.cc04()}</p>
      <label class="mt-4 block max-w-md">
        <span class="flex items-center justify-between font-mono text-[11px] uppercase tracking-wider text-muted-foreground"><span>{m.cc05()}</span><span class="kpi-value text-foreground">{codShare}%</span></span>
        <input type="range" min="0" max="100" step="1" bind:value={codShare} oninput={() => run()} aria-label={m.ax22()} class="mt-2 w-full accent-[var(--bitcoin)]" />
      </label>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{m.cc06()}</p>
        <p class="kpi-value mt-2 font-heading text-2xl">{rpT(res.cashFloatIdr)}</p>
        <p class="mt-1 text-xs text-muted-foreground">COD {numId(res.basis.codDailyM, 2)}{m.cc3f1()}</p>
      </div>
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{m.cc07()}</p>
      ...
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{m.cc08()}</p>
        <p class="kpi-value mt-2 font-heading text-2xl text-success-foreground">{rpM(res.risk.discrepancyCostSavedIdr)}</p>
        <p class="mt-1 text-xs text-muted-foreground">{m.cc09()}</p>
      </div>
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{m.cc10()}</p>
        <p class="kpi-value mt-2 font-heading text-2xl text-[var(--bitcoin)]">−{numId(res.risk.riskReductionPct, 1)}%</p>
        <p class="mt-1 text-xs text-muted-foreground">{numId(res.risk.reconHoursSavedPerDay, 0)} {m.cc3f2()}</p>
      </div>
    </div>

    <div class="rounded-2xl border border-border bg-card p-5">
      <p class="mb-3 text-sm font-medium">{m.cc11()}</p>
      {#if riskChart}<EChart option={riskChart} height={260} label={m.kc2t8()} />{/if}
    </div>

    <div class="rounded-2xl border-l-4 border-[var(--bitcoin)] bg-muted/30 p-4 text-sm">
      <span class="font-medium">{m.cc12()}</span> {m.cc13()} <strong>{codShare}%</strong> {m.cc14()} <strong>{rpT(res.cashFloatIdr)}</strong> {m.cc15()} <strong>{numId(res.risk.riskReductionPct, 0)}%</strong>. AOV Rp{numId(res.basis.avgOrderValueIdr, 0)} {m.cc3f3()} {res.note}
    </div>
  {:else}
