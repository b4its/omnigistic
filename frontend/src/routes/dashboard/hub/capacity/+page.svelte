<script lang="ts">
  ...
  async function load(userTriggered = false) {
    failed = false;
    loaded = false;
    if (userTriggered) notify({ message: m.hc2t1(), type: "info", title: "Capacity Alert" });
    try {
      hubs = await api.hubs();
      if (userTriggered) notify({ message: m.hc2t2(), type: "success", title: "Capacity Alert" });
    } catch {
      hubs = [];
      failed = true;
      if (userTriggered) notify({ message: m.hc2t3(), type: "error", title: "Capacity Alert" });
    }
    loaded = true;
  }
  ...
  const TIERS: Array<{ key: Tier; label: string; demandM: number; note: string }> = [
    { key: "lembah", label: "Lembah", demandM: 78, note: m.hc2t4() },
    { key: "normal", label: "Normal", demandM: 92.5, note: m.hc2t5() },
    { key: "puncak", label: "Puncak", demandM: 105, note: m.hc2t6() }
  ];
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="font-heading text-xl font-semibold tracking-tight">{m.hc01()}</h1>
  ...
  {#if loaded && failed}
    <div class="rounded-2xl border border-destructive/40 bg-destructive/5 p-6 text-center">
      <p class="text-sm font-semibold text-foreground">{m.hc02()}</p>
      <p class="mt-1 text-xs text-muted-foreground">{m.hc03()}</p>
      <button type="button" onclick={() => load(true)} class="mt-3 rounded-full bg-[var(--primary)] px-4 py-2 text-xs font-semibold text-[var(--primary-foreground)]">{m.hc04()}</button>
    </div>
  {:else if loaded && bdg}
    <div class="rounded-2xl border border-border bg-card p-5">
      <p class="text-xs text-muted-foreground">{m.hc05()}</p>
    ...
    <div>
      <p class="mb-2 text-sm font-medium">{m.hc06()}</p>
      <div class="grid gap-4 sm:grid-cols-3">
        {#each TIERS as t (t.key)}
          ...
          >
            ...
            {#if tier === t.key}<p class="mt-1 text-[11px] font-semibold text-primary">{m.hc07()}</p>{/if}
          </button>
        {/each}
      </div>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="text-xs text-muted-foreground">{m.hc3f1()}{THRESHOLD}%</p>
      ...
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="text-xs text-muted-foreground">{m.hc08()}</p>
        <p class="kpi-value text-2xl">{peak ? peak.util.toFixed(1) + "%" : "—"}</p>
        <p class="text-xs text-muted-foreground">{peak?.name ?? "—"}</p>
      </div>
      <div class="rounded-2xl border border-border bg-card p-4">
        <p class="text-xs text-muted-foreground">{m.hc09()}</p>
        <p class="kpi-value text-2xl">{overflowTotalM > 0 ? overflowTotalM.toFixed(3) + "M" : "0"}</p>
        <p class="text-xs text-muted-foreground">{m.hc10()}</p>
      </div>
    </div>

    <div class="rounded-2xl border border-border bg-card p-5">
      <div class="mb-3 flex items-center justify-between">
        <p class="text-sm font-medium">{m.hc3f2()}{THRESHOLD}{m.hc3f3()} {TIERS.find((x) => x.key === tier)?.label}</p>
        <span class={cn("rounded-md px-3 py-1.5 text-xs font-semibold", breaches.length ? "bg-destructive text-destructive-foreground" : "bg-success text-success-foreground")}>
          {breaches.length ? m.hc3t1({ n: breaches.length }) : m.hc3t2()}
        </span>
      </div>
      <ul class="space-y-2">
        {#if breaches.length === 0}
          <li class="text-sm text-muted-foreground">{m.hc11()}</li>
        {/if}
      ...
      <p class="mt-3 text-xs text-muted-foreground">
        {m.hc3f4()}{scale.toFixed(3)} {m.hc3f5()} {BASE_DEMAND_M.toLocaleString("id-ID")}M/bulan
        {m.hc3f6()}
        {#if bdg}{m.hc3f7()} {bdg.utilizationPct}% → {(bdg.utilizationPct * scale).toFixed(1)}%.{/if}
      </p>
    </div>
  {:else if loaded}
    <div class="rounded-2xl border border-dashed border-border bg-card p-6 text-center text-sm text-muted-foreground">{m.hc12()}</div>
  {:else}
