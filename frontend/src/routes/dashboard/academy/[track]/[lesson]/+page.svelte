                ...
                {#each q.options as opt, oi (oi)}
                  ...
                  >
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border {isSel ? 'border-primary bg-primary text-primary-foreground' : 'border-border'}">
                      ...
                      {:else if isSel}<span class="h-1.5 w-1.5 rounded-full bg-[var(--primary)]"></span>{/if}
                    </span>
        ...
        <div class="mt-6 flex flex-wrap items-center gap-3">
          {#if !submitted}
            <button
              ...
              class="inline-flex items-center gap-2 rounded-full bg-[var(--primary)] px-5 py-2.5 text-sm font-semibold text-[var(--primary-foreground)] transition-transform hover:-translate-y-px disabled:cursor-not-allowed disabled:opacity-40"
            >
